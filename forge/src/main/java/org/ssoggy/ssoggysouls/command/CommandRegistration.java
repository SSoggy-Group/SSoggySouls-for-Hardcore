...
public class CommandRegistration {
            ...
            .executes(context -> {
                context.getSource().sendFailure(MessageUtil.get("usage-revive").copy()
                    .withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED)
                        ...
                        .withHoverEvent(new net.minecraft.network.chat.HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, MessageUtil.get("click-to-autofill").copy().withStyle(net.minecraft.ChatFormatting.GRAY) /* NO-SONAR */))));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
    ...
    private static void executeRevive(String targetName, CommandSourceStack source) {
        ...
        int defaultLives = org.ssoggy.ssoggysouls.util.ConfigManager.getConfig().getDefaultLives(); // NO-SONAR
