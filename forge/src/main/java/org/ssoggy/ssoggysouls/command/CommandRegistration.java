...
public class CommandRegistration {
    ...
    private static void registerReviveCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                var msg = MessageUtil.get("usage-revive").copy();
                var click = new net.minecraft.network.chat.ClickEvent(net.minecraft.network.chat.ClickEvent.Action.SUGGEST_COMMAND, "/revive ");
                var hoverTxt = MessageUtil.get("click-to-autofill").copy().withStyle(net.minecraft.ChatFormatting.GRAY);
                var hover = new net.minecraft.network.chat.HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, hoverTxt);
                context.getSource().sendFailure(msg.withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED).withClickEvent(click).withHoverEvent(hover)));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
    ...
    private static void executeRevive(String targetName, CommandSourceStack source) {
        ...
        int livesDef = org.ssoggy.ssoggysouls.util.ConfigManager.getConfig().getDefaultLives();
        boolean success = db.revivePlayer(targetData.getUuid(), livesDef);
        if (success) {
    ...
    private static void registerSetLivesCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("psetlives")
            .requires(source -> source.hasPermission(2))
            .executes(context -> {
                var psetlivesMsg = MessageUtil.get("usage-psetlives").copy();
                var clickEv = new net.minecraft.network.chat.ClickEvent(net.minecraft.network.chat.ClickEvent.Action.SUGGEST_COMMAND, "/psetlives ");
                var hoverEvTxt = MessageUtil.get("click-to-autofill").copy().withStyle(net.minecraft.ChatFormatting.GRAY);
                var hoverEv = new net.minecraft.network.chat.HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, hoverEvTxt);
                context.getSource().sendFailure(psetlivesMsg.withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED).withClickEvent(clickEv).withHoverEvent(hoverEv)));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
