...
## 2024-06-25 - [Java Stream to Collection Incompatibility]
**Learning:** `Collections.unmodifiableSet(...)` requires a `Set` interface, but attempting to collect a stream into a specific concrete implementation (e.g. `LinkedHashSet`) using `Collectors.toCollection(...)` returns a `Collection`, not a `Set`. This causes compiler type inference errors (`Collection<Material> does not conform to Set<? extends T>`).
**Action:** When creating unmodifiable sets from streams, use `Collectors.toSet()` if deterministic order isn't strictly required, or manually initialize the set instead of chaining `Collections.unmodifiableSet` with `Collectors.toCollection`.
