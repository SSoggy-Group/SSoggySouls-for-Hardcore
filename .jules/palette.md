...
## 2026-07-22 - [Interactive CLI Config Errors]
**Learning:** When users provide invalid configuration keys (like timers, structures, or materials) in complex commands, static error messages like "Invalid BlockMaterial entered" force them to completely retype the command. Wrapping the invalid input in a clickable component that auto-fills the base command drastically reduces friction.
**Action:** Replace static validation error strings with clickable MiniMessage recovery paths (e.g. `buildErrorComponent`) to allow instant command correction.
