...
## 2024-08-16 - [Interactive CLI Name Linking in Social Command]
**Learning:** Players often want to take follow-up actions (like checking status) on users listed in chat output. This was already learned for Obituaries, but the trust list generated by `/trust info` in `SocialCommand` also outputs player names as plain text. By making usernames clickable with a `suggest_command`, we reduce the friction of typing out another command manually.
**Action:** When displaying lists of players or events involving players in chat, wrap the usernames in a clickable component that suggests a logical follow-up command (e.g., `/pstatus`).

## 2026-08-16 - [Interactive CLI Number Parsing Errors with RPCommandOutput] (NeoForge/Forge CI Fix)
**Learning:** When trying to update the legacy error markers to use Kyori Adventure MiniMessage components and applying hover/click styles in the Forge and NeoForge implementations, calling `.withStyle(...)` directly on the interface `net.minecraft.network.chat.Component` causes compilation errors because the modern Forge/NeoForge API treats `Component` as immutable. You must call `.copy()` first to get a `MutableComponent`. This is the same learning as Fabric but applied to the Forge ecosystem.
**Action:** When porting chat interactivity or applying `.withStyle(...)` to an existing `Component` object in Forge or NeoForge, always insert `.copy()` first.
