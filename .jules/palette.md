...
## 2026-07-11 - [Interactive CLI Number Parsing Errors with RPCommandOutput] (Fabric CI Fix)
**Learning:** In Fabric and Forge/NeoForge, appending Kyori miniMessage styling directly onto a component generated by another object or utility can cause immutability errors or compiler method mismatch errors if the method signature expects a copy or a mutable builder format.
**Action:** When porting Kyori miniMessage styling to components (like `MessageUtil.get().withStyle()`) in Forge or NeoForge, always append `.copy()` first before modifying styles to prevent `cannot find symbol` or mutability compilation failures.
## 2026-07-11 - [SonarCloud Command Style Duplication]
**Learning:** Repetitive component styling chains (e.g., `.withStyle()`, `.withClickEvent()`, `.withHoverEvent()`) in Brigadier command responses trigger SonarCloud "Duplication on New Code" Quality Gate failures when used multiple times in a class.
**Action:** Extract complex chained style or component builders into dedicated static helper methods (e.g., `createAutofillStyle()`) to satisfy SonarCloud duplication metrics while maintaining interactivity.
