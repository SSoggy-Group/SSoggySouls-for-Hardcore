...
## 2026-09-02 - [Interactive CLI Name Linking in Lists]
**Learning:** Players often want to take follow-up actions (like checking status) on users listed in chat output (like trust lists). By making usernames clickable with a `suggest_command`, we reduce the friction of typing out another command manually.
**Action:** When displaying lists of players or events involving players in chat, wrap the usernames in a clickable component using Kyori MiniMessage (e.g., `<click:suggest_command:'/pstatus <player>'>`) to suggest a logical follow-up command.
