...
## 2024-08-03 - String Parsing Optimization in High-Frequency Events
**Learning:** In high-frequency event listeners like command preprocessing (`PlayerCommandPreprocessEvent` / `ServerChatEvent`), using `.toLowerCase().split("\\s+")` is a performance anti-pattern. It unnecessarily compiles a regex pattern and allocates an array of strings for every single typed command. Furthermore, using a simple `.indexOf(' ')` can break with tab formatting (`\t`) passed by proxies like Velocity/BungeeCord.
**Action:** Instead of `split("\\s+")` or `indexOf(' ')`, implement a manual `for` / `while` loop over the string's characters using `Character.isWhitespace()` to find the start and end indices of the first word (the command). Then, use a single `.substring()` call to extract just the base command before invoking `.toLowerCase()`. Apply this optimization uniformly across Paper, Fabric, Forge, and NeoForge event handlers to save allocations.
