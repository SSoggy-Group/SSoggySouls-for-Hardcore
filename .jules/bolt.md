...
## 2024-06-10 - [O(N) vs O(1) in Custom Schedulers]
**Learning:** Custom schedulers using `ConcurrentLinkedQueue` for task management suffer from O(N) lookup times during task cancellation operations because they must iterate through all scheduled tasks to match IDs. Under heavy load, frequent cancellation creates a performance bottleneck. Additionally, assigning IDs using primitive `int` with post-increment in thread-safe contexts introduces race conditions.
**Action:** Always prefer `ConcurrentHashMap<Integer, ScheduledTask>` for task registries to guarantee O(1) lookups during cancellation. Iteration through `tasks.values().iterator()` handles tick removal safely. Replace primitive counters with `AtomicInteger` to ensure thread-safe ID generation.
