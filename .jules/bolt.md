...
## 2024-05-24 - [Avoid synchronized storage lookups by short-circuiting in loops]
**Learning:** Inside `DlcDeaths.visibleDeaths`, `new DlcSocial().getRelationTo()` is called for every death record in a loop. `getRelationTo` triggers synchronous disk reads/property lookups (`DlcServices.socialStorage().getValue()`). For older deaths, this check is entirely unnecessary if the death time already satisfies the global "public" visibility threshold. Additionally, time threshold calculations like `Instant.now().minusSeconds(...)` were unnecessarily recalculated inside the loop body.
**Action:** Always hoist invariant variable allocations (like threshold calculations) outside of loops. Always reorder loop conditions to evaluate cheap checks (like time comparisons) first, allowing you to use `continue` to short-circuit and completely bypass expensive state or storage lookups.
