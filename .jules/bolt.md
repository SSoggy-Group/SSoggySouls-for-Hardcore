...
## 2026-08-02 - [Avoid indexOf(' ') for string parsing tab delimited strings]
**Learning:** Using `indexOf(' ')` to isolate commands breaks on inputs with tabs (e.g. from RCON/proxies). Replacing it with `.split("\s+")` fixes it but causes unnecessary regex compilation and string array allocations on every chat message.
**Action:** Use a manual `for` loop with `Character.isWhitespace()` to find the delimiter index, then use `substring()` to isolate the target string before lowercasing.
