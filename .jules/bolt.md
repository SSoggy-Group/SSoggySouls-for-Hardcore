...
## $(date +%Y-%m-%d) - SonarCloud False Positive NPEs and String Duplication
**Learning:** SonarCloud can flag `Component.copy()` as a Reliability (Bug) failure (NPE risk) if the upstream `MessageUtil.get()` is not explicitly annotated with `@NotNull`, even if it safely returns `Component.empty()`. Additionally, repeating translation keys like "click-to-autofill" more than 3 times within a file triggers the `java:S1192` String Duplication rule.
**Action:** When chaining styles onto `MessageUtil.get()`, always annotate utility methods returning non-null wrappers (like `Component` or `Text`) with `@NotNull`. Extract repeated string keys to `private static final String` to satisfy deduplication rules cleanly.
