        ...
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                context.getSource().sendFailure(MessageUtil.get("usage-revive")
                    .copy().withStyle(style -> style.withColor(net.minecraft.ChatFormatting.RED)
                ...
                CompletableFuture.runAsync(() -> {
                    ...
                    source.getServer().execute(() -> {
                        switch (result.type) {
                            ...
                            case SUCCESS -> {
                                source.sendSystemMessage(Component.literal("--- Recent Admin Logs ---")
                                        .copy().withStyle(net.minecraft.ChatFormatting.RED, net.minecraft.ChatFormatting.BOLD));
                                if (source.isPlayer()) {
                                    for (String line : result.lines) {
                                        source.sendSystemMessage(Component.literal(line).copy().withStyle(style ->
                                            style.withColor(net.minecraft.ChatFormatting.GRAY)
