...
public class CommandRegistration {
    ...
    private static net.minecraft.network.chat.MutableComponent createInteractiveError(String messageKey, String suggestCommand) {
        return MessageUtil.get(messageKey).copy()
    ...
    private static final String USAGE_REVIVE = "usage-revive";
    private static final String USAGE_PSETLIVES = "usage-psetlives";
    private static final String USAGE_PSETLIVES_PLAYER = "usage-psetlives-player";
    ...
    private static void registerReviveCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                context.getSource().sendFailure(createInteractiveError(USAGE_REVIVE, "/revive "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
    ...
    private static void registerSetLivesCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("psetlives")
            .requires(source -> source.hasPermission(2))
            .executes(context -> {
                context.getSource().sendFailure(createInteractiveError(USAGE_PSETLIVES, "/psetlives "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
                ...
                .executes(context -> {
                    String targetName = StringArgumentType.getString(context, PLAYER);
                    context.getSource().sendFailure(createInteractiveError(USAGE_PSETLIVES_PLAYER, "/psetlives " + targetName + " "));
                    return 0;
                })
