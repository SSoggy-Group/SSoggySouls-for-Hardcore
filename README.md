...
git clone https://github.com/SSoggy-Group/SSoggySouls-for-Hardcore.git
cd SSoggySouls-for-Hardcore

# Build one platform
./gradlew -DonlyPlatform=paper :paper:build
./gradlew -DonlyPlatform=fabric :fabric:build
./gradlew -DonlyPlatform=neoforge :neoforge:build

# Run local dev servers where supported
./gradlew -DonlyPlatform=fabric :fabric:runServer
./gradlew -DonlyPlatform=neoforge :neoforge:runServer

# Run tests for one platform
./gradlew -DonlyPlatform=paper :paper:test
./gradlew -DonlyPlatform=fabric :fabric:test
./gradlew -DonlyPlatform=neoforge :neoforge:test
```

Built JARs are written to each platform's `build/libs/` folder. For Paper, copy `paper/build/libs/*.jar` into a local Paper server's `plugins/` folder and start the server. For Fabric or NeoForge, copy the matching JAR into a local server's `mods/` folder.

Use `-DonlyPlatform=<platform>` when running Gradle locally. It keeps platform-specific Gradle plugins isolated from each other. Forge currently fails to configure on macOS with `Could not find lwjgl-freetype-3.3.3-natives-macos-patch.jar`; use the release JAR for local Forge testing until that dependency issue is fixed.
...
2. Put the same database credentials in both server configs
...
- **Head drop config options** - Added `head-place-as-block`, `head-no-despawn`, and `head-fireproof` to `config.yml` so you can control exactly how heads behave on death. See [Head Mechanics](#player-head-drops) for details.
