    ...
    public RPStorage(JavaPlugin plugin, String fileName) {
        ...
        if (!file.exists()) {
            try {
                ...
                logger.log(Level.SEVERE, "Could not create storage file: {0}", new Object[]{file.getPath()});
    ...
    public void saveConfig() {
        ...
        ioExecutor.execute(() -> {
            try {
                ...
                logger.log(Level.SEVERE, "Could not save configuration to {0}", new Object[]{file.getPath()});
                logger.log(Level.SEVERE, "Exception details:", e);
            }
        });
    }
